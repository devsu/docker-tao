...
if [ ! -f "/var/www/html/config/generis.conf.php" ]; then
    ...
    php -d max_execution_time=300 /var/www/html/tao/scripts/taoInstall.php --file_path $FILE_PATH --db_driver $DB_DRIVER --db_host $DB_HOST --db_name $DB_NAME --db_user $DB_USER --db_pass $DB_PASSWORD --module_namespace $URL/first.rdf --module_url $URL --user_login $USER --user_pass $PASSWORD -e taoCe
